...
> = {
  ...
  body_fat_percent: {
    title: 'אחוז שומן',
    text: 'אם אין – השאירו ריק. ההיקפים יעזרו להבין מגמות. המערכת תחשב אוטומטית אם יוזנו היקפים מתאימים.',
  },
};
...
export default function ProfilePage() {
  ...
  // State לזיהוי חישוב אוטומטי אחרון (מונע דריסה אם המשתמש הזין ידנית משהו אחר)
  const [lastAutoBf, setLastAutoBf] = useState<number | null>(null);
  ...
  // ----- חישוב שומן אוטומטי לפי שיטת הצי האמריקאי (Live) -----
  const calculatedNavyBf = useMemo(() => {
    if (!profile.height_cm || profile.height_cm <= 0) return null;

    if (profile.gender === 'male') {
      const neckCm = meas.neck_cm;
      const waistCm = meas.waist_navel_cm;
      
      if (neckCm && waistCm && waistCm > neckCm) {
        const bf = 495 / (1.0324 - 0.19077 * Math.log10(waistCm - neckCm) + 0.15456 * Math.log10(profile.height_cm)) - 450;
        const finalBf = Number(bf.toFixed(1));
        
        // מוודאים שהתוצאה הגיונית אנטומית (מונע הזנה של שגיאות הקלדה)
        if (finalBf >= 2 && finalBf <= 60) return finalBf;
      }
    } else if (profile.gender === 'female') {
      const neckCm = meas.neck_cm;
      const waistCm = meas.waist_narrow_cm;
      const hipsCm = meas.hips_cm;
      
      if (neckCm && waistCm && hipsCm && (waistCm + hipsCm > neckCm)) {
        const bf = 495 / (1.29579 - 0.35004 * Math.log10(waistCm + hipsCm - neckCm) + 0.22100 * Math.log10(profile.height_cm)) - 450;
        const finalBf = Number(bf.toFixed(1));
        
        if (finalBf >= 5 && finalBf <= 60) return finalBf;
      }
    }
    return null;
  }, [profile.height_cm, profile.gender, meas.neck_cm, meas.waist_navel_cm, meas.waist_narrow_cm, meas.hips_cm]);

  // עדכון השדה באופן אוטומטי במידה ומחושב ערך חדש
  useEffect(() => {
    if (calculatedNavyBf !== null) {
      // מעדכנים רק אם השדה ריק, או אם הערך הנוכחי הוא הערך האוטומטי הקודם שחישבנו
      // כך אנו לא דורסים הקלדה חופשית של המשתמש אם הוא מזין ידנית
      if (meas.body_fat_percent === null || meas.body_fat_percent === lastAutoBf) {
        setMeas((prev) => ({ ...prev, body_fat_percent: calculatedNavyBf }));
        setLastAutoBf(calculatedNavyBf);
      }
    }
  }, [calculatedNavyBf, meas.body_fat_percent, lastAutoBf]);
  ...
  const addMeasurement = async (e: React.FormEvent) => {
    ...
    // ----- בדיקת חוסרים לחישוב אחוז שומן אוטומטי -----
    if (meas.body_fat_percent === null) {
      const missingFields: string[] = [];
      
      if (profile.gender === 'unspecified' || profile.gender === 'other') {
        missingFields.push('מין (יש לבחור זכר או נקבה בטאב הפרופיל)');
      }
      if (!profile.height_cm) {
        missingFields.push('גובה (יש להזין בטאב הפרופיל)');
      }

      // בדיקה מותאמת אישית למין
      if (profile.gender === 'male' || profile.gender === 'unspecified' || profile.gender === 'other') {
        if (!meas.neck_cm) missingFields.push('היקף צוואר');
        if (!meas.waist_navel_cm) missingFields.push('היקף בטן (בקו הטבור)');
      }
      if (profile.gender === 'female') {
        if (!meas.neck_cm) missingFields.push('היקף צוואר');
        if (!meas.waist_narrow_cm) missingFields.push('היקף מותניים (החלק הצר)');
        if (!meas.hips_cm) missingFields.push('היקף ירכיים (החלק הרחב ביותר)');
      }

      if (missingFields.length > 0) {
        const confirmMsg = `שים לב! לא הוזן אחוז שומן.\n\nהמערכת מסוגלת לחשב עבורך את אחוז השומן אוטומטית (לפי שיטת הצי האמריקאי) במידה ותשלים את הנתונים החסרים הבאים:\n\n${missingFields.map(f => `- ${f}`).join('\n')}\n\nהאם להמשיך להוסיף את המדידה ללא אחוז שומן?`;
        if (!window.confirm(confirmMsg)) {
          return; // עוצר את השמירה ומאפשר למשתמש להזין את הנתונים
        }
      }
    }
    ...
    // איפוס הטופס והזיכרון של חישוב האוטומטי
    setMeas({
